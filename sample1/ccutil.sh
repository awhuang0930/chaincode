...
function    usage {
    echo  "Usage: ./ccutil.sh    install | instantiate | invoke | query "
    echo  "Installs the Node CC to specified Organization"
}

export CC_CONSTRUCTOR='{"Args":["InitLedger"]}'
# export CC_NAME="nodecc"
export CC_NAME="nodecc3"
export CC_PATH="/var/hyperledger/nodechaincode/sample1"
...
case $OPERATION in
    ...
    "invoke")
            echo "Invoke asset transfer to other owner"
            peer chaincode invoke -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"function":"TransferAsset", "Args":["asset6","May"]}'
        ;;
